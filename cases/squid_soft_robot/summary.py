...
def _context_value(context: Mapping[str, object], name: str):
    try:
        ...
        raise KeyError(f"build_sharp_case_run_report missing context value: {name}") from exc
...
def build_sharp_case_run_report(context: Mapping[str, object]) -> dict[str, object]:
    ...
    max_fsi_coupling_aitken_update_count = (
        max(
            int(
                row.get("fsi_coupling_aitken_update_count", 0)
                or 0
            )
            for row in rows
        )
