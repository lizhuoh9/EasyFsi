...
def _projected_ibm_probe_distance_m(
    fluid: CartesianFluidSolver,
    spacing_m: tuple[float, float, float],
) -> float:
    grid = getattr(fluid, "grid", None)
    if grid is not None and not bool(getattr(grid, "is_uniform", True)):
        return 0.0
    return min(float(value) for value in spacing_m)
...
def advance_projected_ibm_region_pair_fluid_step(
    ...
    probe_distance_m = _projected_ibm_probe_distance_m(fluid, spacing_m)
    ...
    def apply_correction_pass(
        *,
        read_full_reports: bool,
        read_force_pair_report: bool,
        final_correction_pass: bool,
    ):
        ...
        applied_force_report = pass_force_report
        if device_force_impulse:
        ...
        if pressure_robin_matrix_enabled and (
            final_correction_pass or read_full_reports
        ):
    ...
    for _fluid_substep in range(fluid_substeps):
        ...
        for _correction_iteration in range(correction_iterations):
            ...
            (
                ...
                read_force_pair_report=read_force_pair_report,
                final_correction_pass=final_correction_pass,
            )
            if not device_force_impulse:
                if applied_force_report is None:
                    raise RuntimeError("projected IBM force pass did not return a force report")
                for component in range(3):
                    primary_fluid_impulse_n_s[component] += (
                        float(applied_force_report.primary_fluid_force_n[component])
                        * correction_dt_s
                    )
                    secondary_fluid_impulse_n_s[component] += (
                        float(applied_force_report.secondary_fluid_force_n[component])
                        * correction_dt_s
                    )
