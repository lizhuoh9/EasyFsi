...
class TriSurfaceRegionDiagnostics:
    ...
    def __init__(self, face_capacity: int, runtime: TaichiRuntimeConfig | None = None):
        ...
        self.report_force_cell_hash_capacity = max(
            1,
            self.report_force_cell_index_capacity * 2,
        )
        self.report_force_cell_hash_keys = ti.field(
            dtype=ti.i64,
            shape=self.report_force_cell_hash_capacity,
        )
        self.report_force_cell_hash_force_n = ti.Vector.field(
            3,
            dtype=ti.f32,
            shape=self.report_force_cell_hash_capacity,
        )
    ...
    def load_faces(
        ...
        padded_normals = np.zeros((self.face_capacity, 3), dtype=np.float32)
        padded_areas = np.zeros((self.face_capacity,), dtype=np.float32)
        padded_regions = np.zeros((self.face_capacity,), dtype=np.int32)
        padded_centroids[: self.face_count] = centroids
        padded_normals[: self.face_count] = normals
        padded_areas[: self.face_count] = areas
        padded_regions[: self.face_count] = regions
        self.centroid_m.from_numpy(padded_centroids)
        self.rest_centroid_m.from_numpy(padded_centroids)
        self.normal.from_numpy(padded_normals)
        self.area_m2.from_numpy(padded_areas)
    ...
    def _pack_report_only_active_force_cell_count(self):
        for slot in range(ti.static(self.report_force_cell_hash_capacity)):
            self.report_force_cell_hash_keys[slot] = 0
            self.report_force_cell_hash_force_n[slot] = ti.Vector([0.0, 0.0, 0.0])
        stored_count = ti.min(
        ...
        for index in range(stored_count):
            ...
            if linear_index != 0:
                slot = linear_index % ti.static(self.report_force_cell_hash_capacity)
                probe_count = 0
                inserted = False
                while (
                    probe_count < ti.static(self.report_force_cell_hash_capacity)
                    and not inserted
                ):
                    stored_index = self.report_force_cell_hash_keys[slot]
                    if stored_index == 0:
                        self.report_force_cell_hash_keys[slot] = linear_index
                        self.report_force_cell_hash_force_n[slot] = (
                            self.report_force_cell_force_n[index]
                        )
                        inserted = True
                    elif stored_index == linear_index:
                        self.report_force_cell_hash_force_n[slot] += (
                            self.report_force_cell_force_n[index]
                        )
                        inserted = True
                    slot = (slot + 1) % ti.static(self.report_force_cell_hash_capacity)
                    probe_count += 1
        active_count = 0
        for slot in range(ti.static(self.report_force_cell_hash_capacity)):
            if (
                self.report_force_cell_hash_keys[slot] != 0
                and self.report_force_cell_hash_force_n[slot].norm() > 0.0
            ):
                active_count += 1
        ...
        for face in range(face_count):
            ...
            if region == primary_region_id or region == secondary_region_id:
                ...
                if impedance_ns_m > 0.0 and interface_area_m2 > 0.0:
                    ...
                    sample_probe = (
                        self.centroid_m[face] + n * effective_probe_distance_m
                    )
                    in_bounds = (
