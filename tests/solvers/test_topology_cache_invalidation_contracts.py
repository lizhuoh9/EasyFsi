...
class TopologyCacheInvalidationContracts(unittest.TestCase):
    ...
    def test_convert_row_cloud_orphans_invalidates_both_caches_before_device_write(
        self,
    ) -> None:
        solver = self._solver_with_published_caches()
        ...
        solver._reset_row_cloud_orphan_cleanup_report_kernel = lambda: None

        def invoke(observed: list[tuple[bool, bool]]) -> None:
            solver._convert_row_cloud_raw_singletons_kernel = self._raising_mutation(
                solver,
                observed,
            )
            with self.assertRaises(_SyntheticDeviceMutation):
